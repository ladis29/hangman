...
public class Hangman {
	/*
	 * Jogo da forca(hangman) Passado como exercício de aprendizagem do curso da
	 * Academia do Java, VOFFICE pelo instrutor Vinny Aluno: Ladislau Felisbino
	 */
	...
	public static void main(String[] args) {
		...
		System.out.println(
				"No primeiro vc, " + nome + ", tera que tentar acertar uma entre as 50 palavras reservadas do java,");
		System.out.println("No segundo nivel uma entre as 10 palavras reservadas do java relacionadas a tipos,");
		System.out.println(
				"Isto envolve os tipos primitivos, o unico operador que e palavra reservada e a palavra reservada usada quando um metodo nao retorna valor.");
		...
		System.out.println(
				"No primeiro nivel voce podera errar ate 5 letras, no sexto erro o personagem e enforcado e o jogo acaba.");
		 ...
		 * OBS.: Eu sei que poderia inicializar o array de strings do nível 2 apenas em
		 * caso de o jogador passar para o nível 2 mas nesta fase do desenvolvimento o
		 * impacto gerado por essa alocação de memória não impacta no desempenho do jogo
		 * portanto preferi deixar assim e me preocupar mais em fazer funcionar!!
		 */

		String[] wordsLevel1 = { "abstract", "assert", "boolean", "break", "byte", "case", "catch", "char", "class",
				"const", "continue", "default", "do", "double", "else", "enum", "extends", "final", "finally", "float",
				"for", "goto", "if", "implements", "import", "instanceof", "int", "interface", "long", "native", "new",
				"package", "private", "protected", "public", "return", "short", "static", "strictfp", "super", "switch",
				"synchronized", "this", "throw", "throws", "transient", "try", "void", "volatile", "while" };

		String[] wordsLevel2 = { "boolean", "byte", "char", "double", "float", "instanceof", "int", "long", "short",
				"void" };

		// Criando as variáveis que vão servir de referência para o limite de tentativas
		// de cada nível
		int maxGuesses = 0;
		int level = 1;
		String[] words = {};
		...
		// uma letra que já digitei e está certa
		...
		while (!finished) {
			...
			tries = 0;
			played = 1;
			tipedLetters = new ArrayList<Character>(); //reiniciando o arraylist de caracteres para o level 2
			...
			// pegar a palavra escolhida aleatoriamente e transformar em um array de
			// caracteres
			...
			// Enquanto a palavra não for descoberta e a quantidade de tentativas não
			...
			while (!wordWasDiscovered && tries != maxGuesses) {
				...
				if (tipedLetters.contains(guess)) {
					played++;
					System.out.println("Letra já digitada anteriormente, aperte enter e tente outra");
					sc.nextLine();
					continue;
				} else {
					tipedLetters.add(guess);
				}
				tries++;
				played++;
				...
				if (guess == '_') {
					...
					for (int i = 0; i < wordToGuess.length; i++) {
						...
						if (wordToGuess[i] == guess) {
							...
							// Essa linha a seguir coloquei porque percebi que quando eu acertava a primeira
							// letra
					...
					if (isTheWordGuessed(showedSpaces)) {
						...
						if (level == 1) {
							System.out.println(
									"No segundo nivel voce podera errar ate 4 letras, no quinto erro o personagem e enforcado e o jogo tambem acaba.");
	...
	// Método que verifica se todos os '_'(underscores) já foram substituídos por
	...
	// Método para desenhar a forca no console usando "switch", a variável tries pq
	// ela incrementa com a quantidade de erros e o level pra diferenciar
